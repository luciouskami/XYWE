...
function InitHatredSystem takes nothing returns nothing
    set udg__wjz_dn = CreateForce()
    set udg__wjz_wj = CreateForce()
endfunction
...
function aa takes unit damageunit returns nothing
    local integer i
...
function RUN_EHUD takes nothing returns nothing
    call SaveUnitHatred(GetEnumUnit(),GetTriggerUnit(),0.0)
    call DestroyIndex(GetHandleId(GetEnumUnit()),GetHatredUnitIndex(GetEnumUnit(),GetTriggerUnit()))
endfunction
...
function EliminateHatredUnitDead takes nothing returns nothing
    call ForForce( udg__wjz_dn, function Actions_EHUD )
endfunction

function EliminateHatredUnit takes nothing returns nothing
    if ((IsPlayerInForce(GetOwningPlayer(GetTriggerUnit()), udg__wjz_dn) == true)) then
        call FlushMissionHatredSystemByUnit( GetTriggerUnit() )
    else
        if ((IsPlayerInForce(GetOwningPlayer(GetTriggerUnit()), udg__wjz_wj) == true)) then
...
function EliminateHatredAll takes nothing returns nothing
    call ForForce( udg__wjz_dn, function EliminateHatredAllgroup )
endfunction
...
function Skillhatred takes location point, real radius, real factor, real a, real b, boolean B returns nothing
    ...
    loop
        ...
        if ((IsUnitInForce(tempunit, udg__wjz_dn) == true)) then
